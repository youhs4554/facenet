...
output_dir="$repo_root/.tools/audio2face3d/final-taro-test-mrq"
wav="${1:-/home/aim/Downloads/test.wav}"
video_only="$output_dir/Taro_Audio2Face_test_video-only.mp4"
final="$output_dir/Taro_Audio2Face_test_FINAL.mp4"
...
if [[ $(find "$output_dir" -maxdepth 1 -name 'Taro_A2F_test.*.png' | wc -l) -ne 109 ]]; then
  echo "ERROR: expected 109 verified MRQ PNG frames." >&2
  exit 1
fi

"$ffmpeg" -hide_banner -y -loglevel warning \
  -framerate 30 -start_number 0 \
  -i "$output_dir/Taro_A2F_test.%04d.png" \
...
"$ffprobe" -v error \
  ...
  -of json "$final" | tee "$output_dir/Taro_Audio2Face_test_FINAL.ffprobe.json"
...
"$ffmpeg" -hide_banner -nostats -i "$final" -map 0:a:0 \
  ...
  | tee "$output_dir/Taro_Audio2Face_test_FINAL.volume.txt"
sha256sum "$final" | tee "$output_dir/Taro_Audio2Face_test_FINAL.sha256"
